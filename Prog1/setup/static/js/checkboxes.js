...
$('input[type="checkbox"]').on('change', function() {
 ...
 $('#btn_visualizar').add('#btn_editar').add('#btn_apagar').on('click', function() {
	...
	if (coduser != null){
		...
		axios.get('/request/usuario', { params: { 'cod': coduser } })
		.then(response => {
			...
			$('#id_nivelvendedor' ).val(response.data.nivelvendedor	).change();
			...
			if(this.id == 'btn_visualizar' || this.id == 'btn_apagar'){
				if (this.id == 'btn_visualizar'){
					...
					if (response.data.status == '2'){
						alert("Não é permitida exclusão de registros já excluidos/inativos")
						return
					}
				...
				$('#id_tipo'		  ).attr('onchange',"this.value = '" + response.data.tipo 			+ "'");
				$('#id_filial'		  ).attr('onchange',"this.value = '" + response.data.filial 		+ "'");
				$('#id_nivelvendedor' ).attr('onchange',"this.value = '" + response.data.nivelvendedor 	+ "'");
				$('#id_status'		  ).attr('onchange',"this.value = '" + response.data.status 		+ "'");
			}

			else if(this.id == 'btn_editar'){
				// Validar se é possível editar
				if (response.data.status == '2'){
					alert("Não é permitida edição de registros já excluidos/inativos")
					return
				}
...
$('#btn_incluir').click(function() {
	...
	// Atualizar labels da tela
	$('#ModalInclusaoLabel').text('Incluir Usuário');
	$('#id_btnok'		      ).text('Salvar');
