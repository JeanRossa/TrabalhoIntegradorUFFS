...
$('#btn_visualizar')
  ...
  .on('click', function () {
    ...
    nivelfilial = $('input:checkbox[name=checkbox_branchLevels]:checked').attr(
      'id'
    )
    if (nivelfilial != null) {
      ...
      axios
        ...
        .then(response => {
          ...
          $('#id_nivelfilial').val(response.data.cidade.trim())
          $('#id_descricao').val(response.data.estado)
